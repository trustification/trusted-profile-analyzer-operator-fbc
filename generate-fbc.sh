...
{
if [[ "$2" == "brew" ]]; then
    sed -i 's|image: registry.redhat.ioregistry.redhat.io/rhtpa/rhtpa-rhel9-operator|image: brew.registry.redhat.ioregistry.redhat.io/rhtpa/rhtpa-rhel9-operator|g' "${frag}"/graph.yaml
fi
}
...
{
if [[ "$2" == "brew" ]]; then
    sed -i 's|image: brew.registry.redhat.io/rhtpa/rhtpa-rhel9-operator|image: registry.redhat.io/rhtpa/rhtpa-rhel9-operator|g' "${frag}"/graph.yaml
    sed -i 's|brew.registry.redhat.io/rhtpa/rhtpa-rhel9-operator|registry.redhat.io/rhtpa/rhtpa-rhel9-operator|g' "${frag}"/catalog/rhtpa-operator/catalog.json
fi
}
...
case $cmd in
  ...
  "--init-basic")
    ...
    if [ -z "$frag" ]
    then
      echo "Please specify OCP minor, eg: v4.17"
      exit 1
    fi
    ...
    yqOrjq=$3
    mkdir -p "${frag}/catalog/rhtpa-operator/" "${frag}/${frag}"
  ...
  "--render")
    ...
    setBrew "${frag}" "$3"
    "${OPM_CMD}" alpha render-template basic "${frag}"/graph.yaml > "${frag}"/catalog/rhtpa-operator/catalog.json
    unsetBrew "${frag}" "$3"
  ;;
  "--render-all")
    for f in ./"v4."*; do
  ...
  "--comment-graph")
    ...
    grep -E "^image: [brew\.]*registry.redhat.io/rhtpa/rhtpa-rhel9-operator[-rhel9]*@sha256" "$frag"/graph.yaml | while read -r line ; do
      image=${line/image: /}
